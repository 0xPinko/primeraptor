...
SUBJECT="Velociraptor - Access Information"
BODY=$(cat <<EOF
<html>
  <body>
    <p>Hi,</p>
    <p>Please find below the details for your Velociraptor server:</p>
    <ul>
      ...
      <li><strong>Client files:</strong> <a href="$PRESIGNED_URL">$PRESIGNED_URL</a></li>
    </ul>
    <p>Thanks,</p>
  </body>
</html>
...
aws ses send-email --from "$FROM_EMAIL" --destination "ToAddresses=$TO_EMAIL" \
--message "Subject={Data='$SUBJECT',Charset=utf-8},Body={Html={Data='$(echo "$BODY" | sed 's/\"/\\\"/g')',Charset=utf-8}}" --region "$AWS_REGION"